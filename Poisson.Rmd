---
title: 'HW4: Team [Team 2]'
author: '[Jialei Guo, Tun-Chieh Hsu, Xialingzi Jin, Ziying Sun]'
...
College.train = College.train[,-17]

ggpairs(data = College.train[c(2, 1,3:7)], 
        mapping = ggplot2::aes(y = Apps),
        lower = list(continuous = wrap("points", alpha = 0.3), combo = wrap("dot_no_facet", alpha = 0.4)))

ggpairs(College.train[c(2, 8:12)], 
        mapping = ggplot2::aes(y = Apps),
        lower = list(continuous = wrap("points", alpha = 0.3), combo = wrap("dot_no_facet", alpha = 0.4)))

ggpairs(College.train[c(2, 13:17)], 
        mapping = ggplot2::aes(y = Apps),
        lower = list(continuous = wrap("points", alpha = 0.3), combo = wrap("dot_no_facet", alpha = 0.4)))
...
College.train = College.train %>% select(-c(Top25perc,Outstate,Expend,P.Undergrad,Terminal))
...
College.test = College.test %>% select(-c(Top25perc,Outstate,Expend,P.Undergrad,Terminal, college))
...
g1 = ggplot(df, aes(x = test_fit1, y = V3)) + 
  geom_point() + 
  ylab("True values") +
  labs(title = "Test results comparison with fit1")
g2 = ggplot(df, aes(y = V3, x = test_best)) + 
  geom_point() + 
  ylab("True values") +
...
ggplot(data = rmse_rep, aes(x = V1)) + 
  geom_histogram() +
  geom_vline(xintercept =rmse_fit, col = "red") +
  geom_vline(xintercept =rmse_fit_test, col = "blue") + 
  labs(title = "Simulation Results of RMSE") +
  theme(plot.title = element_text(hjust = 0.5))
...
ggplot(data = rmse_rep_best, aes(x = V1)) + 
  ...
  geom_vline(xintercept = rmse_best_test, col = "blue") +
  labs(title = "Simulation Results of RMSE") +
  theme(plot.title = element_text(hjust = 0.5))


true_test = College.test$Apps
interval = as.data.frame(predict(lm_fit1, College.test[-2], interval="predict")) %>% select(lwr, upr)
count = 0
for(i in 1: nrow(College.test)){
  if(true_test[i] > interval$lwr[i] & true_test[i] < interval$upr[i]){
    count = count + 1
  }
}
acc_fit1 = count/nrow(College.test)
...
interval = as.data.frame(exp(predict(best_step, ctest1[-2], interval="predict"))) %>% select(lwr, upr)
count = 0
for(i in 1: nrow(College.test)){
  if(true_test[i] > interval$lwr[i] & true_test[i] < interval$upr[i]){
    count = count + 1
...
acc_best = count/nrow(College.test)
```


In the first histogram, although the RMSE of training data is near the center of distribution, the test RMSE is far away from the RMSE estimated using replicated data, which indicates observed RMSE is not accurate compared to the true model and our model doesn't fit well; for the second histogram, both observed RMSE and test RMSE are far away from the distribution of RMSE based on replicated data, which indicates our second model isn't accurate either. In addition, there are many negative values for the simulated data generated by fit_lim2, since it uses linear model without transformation, which is not reasonable for this data set. 
...
par(mfrow = c(2,2))
plot(fit_poisson2)
pchisq(summary(fit_poisson2)$deviance, summary(fit_poisson2)$df.residual, lower.tail = F)
```

We first look the variables summary, which indicates some of the variables have large variance. Thus, we take the log transformation to reduce the variance. 

In fit_poisson2 model summary, we notice that the coefficient estimates, standard error and p-values are very small. All variables are significant after transformation. The residual deviance is 176721 and is very huge. The p-value from the chi-square test suggests that a residual deviance as large or larger than what we observed under the model is highly unlikely. Hence the model is not adequate or lack of fit (overdispersion).

After transformation and add of interactions, assumptions including linearity, normal distribution of residual are basically satisfied. However, from all the plots, especially the leverage plot, we clearly see the evidence that there are several high leverage points for this model and they have significant influence on our fitting result. High leverage points suggests there is great variation among response variable, greater than predicted values, which indicates existence of overdispersion.
...
X <- model.matrix(fit_poisson2)
n_sim <- 1000
sim_fit1 <- sim(fit_poisson2, n_sim)
beta <- sim_fit1@coef
...
for (s in 1:n_sim){
  ...
  y.rep <- rpois(n, mu)
  ct$Apps = y.rep
  fit<- glm(Apps ~  . + (Personal + Books + F.Undergrad)^2 , data = ct, family = 'poisson')
  rmse_rep[s,1] = rmse(ct$Apps, round(fitted(fit)))
}

rmse_fit = rmse(College.train$Apps, fitted(fit_poisson2))


p2test = predict.glm(fit_poisson2, newdata = cTest, type = "response")
rmse_test_p2 = rmse(p2test, College.test$Apps)
...
ggplot(data = rmse_rep, aes(x = V1)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  geom_vline(xintercept =rmse_fit, col = "red") +
  geom_vline(xintercept =rmse_test_p2, col = "blue")
...
Observed RMSE based on training data is far away from distribution of RMSE estimated by simulated test data set. In addition, RMSE for the test data set is even larger. The model does not yield accurate prediction. 
...
Residual deviance $D \sim \chi^{2}_{n-p-1}$ with mean $n-p-1$, and variance $2(n-p-1)$
As degree of freedom goes up,
       $z = \frac {D-(n-p-1)}{\sqrt{2(n-p-1)}} \sim N(0,1)$
Since 95% fall within two standard deviations, we can just calculate $z$ value, if $|z| > 2$, we get the conclusion that there's overdispersion problem.
